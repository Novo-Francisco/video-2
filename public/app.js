...
// ✅ Debug na tela (já mostra que o JS carregou)
function setStatus(html) {
  if (statusEl) statusEl.innerHTML = html;
}
setStatus("✅ app.js carregado. Pronto para gerar vídeo.");

// ✅ Captura erros JS e mostra no #status (já que você não usa Console)
window.addEventListener("error", (e) => {
  setStatus(`<div style="color:#b00020;"><strong>Erro no JavaScript:</strong> ${escapeHtml(e.message)}</div>`);
});
window.addEventListener("unhandledrejection", (e) => {
  setStatus(`<div style="color:#b00020;"><strong>Promise rejeitada:</strong> ${escapeHtml(String(e.reason))}</div>`);
});
...
// ✅ Confere se o HTML tem os elementos certos
if (!btnEl) setStatus(`<div style="color:#b00020;">Botão #generate não encontrado no HTML.</div>`);
if (!promptEl) setStatus(`<div style="color:#b00020;">Campo #prompt não encontrado no HTML.</div>`);
if (!imagesEl) setStatus(`<div style="color:#b00020;">Campo #images não encontrado no HTML.</div>`);
if (!statusEl) alert("Elemento #status não encontrado no HTML.");
  ...
  btnEl.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!Array.isArray(jobs) || jobs.length === 0) {
        throw new Error("A API não retornou jobs. Verifique /generate no backend.");
      }
      ...
      pollingTimer = setInterval(async () => {
        try {
          ...
          if (pollCount > MAX_POLLS) {
            stopPolling();
            disableUI(false);
            setStatus(`<div style="color:#b00020;">Demorou além do esperado. Tente reduzir as imagens.</div>`);
            return;
          }
