...
if (!res.ok) {
throw new Error(data?.error || data?.detail || "generate falhou");
}
return data;
}
...
const data = await readJsonSafe(res);
if (!res.ok) {
throw new Error(data?.error || data?.detail || "status falhou");
}
return data;
}
...
for (let i = 0; i < scenes.length; i++) {
  const s = scenes[i];
  log(`[Cena ${i + 1}/${scenes.length}] Enviando para geração...`);

  const gen = await seedanceGenerate(s);
  const taskId = pick(gen, ["data.task_id", "task_id", "data.taskId", "taskId"]);

  if (!taskId) {
    console.error("Resposta generate:", gen);
    throw new Error("Sem task_id na resposta de generate.");
  }
  ...
  for (let t = 0; t < 200; t++) {
    ...
    if (videoUrl && (status.includes("success") || status.includes("succeed") || status === "completed")) {
      break;
    }

    if (status.includes("fail")) {
      console.error("Resposta status:", st);
      throw new Error(`Cena ${i + 1} falhou.`);
    }

    await new Promise((r) => setTimeout(r, 3000));

    if (t % 5 === 0) {
      log(`[Cena ${i + 1}] aguardando... (${t})`);
    }
  }

  if (!videoUrl) {
    throw new Error(`Timeout: Cena ${i + 1} sem URL.`);
  }

  log(`[Cena ${i + 1}] OK: ${videoUrl}`);
  segmentUrls.push(videoUrl);
}
log("Baixando segmentos para montar o MP4 final no navegador...");
const finalBlob = await concatInBrowser(segmentUrls);
const finalUrl = URL.createObjectURL(finalBlob);

downloadA.href = finalUrl;
downloadA.style.display = "inline-block";
log('Pronto! Clique em "Baixar vídeo final".');
